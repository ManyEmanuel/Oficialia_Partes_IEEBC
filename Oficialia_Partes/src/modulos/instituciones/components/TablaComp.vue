...
const { instituciones } = storeToRefs(institucionStore);

onBeforeMount(() => {
  institucionStore.loadInstituciones();
});
...
const editar = async (id) => {
  $q.loading.show();
  await institucionStore.loadInstitucion(id);
  institucionStore.updateEditar(true);
  //await espera();
...
const eliminar = async (id) => {
  $q.dialog({
    ...
    $q.loading.show();
    const resp = await institucionStore.deleteInstitucion(id);
    if (resp.success) {
      $q.loading.hide();
      $q.notify({
        type: "positive",
