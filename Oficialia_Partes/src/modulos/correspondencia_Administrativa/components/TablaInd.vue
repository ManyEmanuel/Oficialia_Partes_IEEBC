          ...
          <q-tr v-show="props.expand" :props="props">
            <q-td colspan="100%">
              <div class="text-left">
                ...
                {{ props.row.area_asignacion }}
                <br />
                <strong>Áreas con copia: </strong> {{ props.row.area_copia }}
                <br />
...
const { correspondencias, modalAdjuntos } = storeToRefs(
  correspondenciaAdminStore
);
...
onBeforeMount(() => {
  ...
  correspondenciaAdminStore.loadCorrespondencias();
});
...
const columns = [
  ...
  },
  {
    ...
    field: "area_asignacion",
    sortable: false,
  },
  {
    name: "area_copia",
    align: "center",
    label: "Áreas con copias",
    field: "area_copia",
    sortable: false,
  },
  {
...
const buscarFecha = async () => {
  ...
  if (typeof model.value != "string") {
    ...
    resp = await correspondenciaAdminStore.loadCorrespondenciasFecha(from, to);
  } else {
    resp = await correspondenciaAdminStore.loadCorrespondenciasFecha(
      model.value,
      0
    );
  }
...
const asignar = async (id) => {
  $q.loading.show();
  await correspondenciaAdminStore.loadPersonalArea(id);
  correspondenciaAdminStore.actualizarEmpleado(true);
...
const crearReporte = async () => {
  $q.loading.show();
  let resp = await correspondenciaAdminStore.createReporte(
    correspondencias.value,
    textModel.value,
    true
  );
