<template>
  <div class="row">
    <div class="col">
      <q-table
        :rows="correspondenciasGeneral"
        :columns="columns"
        :filter="filter"
        :loading="loading"
        class="columna-fija"
        :pagination="pagination"
        visible-columns="folio,institucion,remitente,fecha_Recepcion,expediente_Oficio,id"
...
const { correspondenciasGeneral, modalAdjuntos } = storeToRefs(
...
onBeforeMount(() => {
  console.log("Entro al before de corresponcencia General");
  correspondenciaAdminStore.loadCorrespondenciaGeneral();
});
...
const buscarFecha = async () => {
  ...
  if (typeof model.value != "string") {
    ...
    resp = await correspondenciaAdminStore.loadCorrespondenciaGeneralFecha(
      from,
      to
    );
  } else {
