...
const actualizarEmpleado = () => {
  correspondenciaStore.actualizarEmpleado(false);
};
...
const asignacion = async () => {
  ...
  if (personalArray.length > 0) {
    resp = await correspondenciaStore.createAsignacionEmpleados(personalArray);
    if (resp.success) {
      $q.notify({
        type: "positive",
        message: resp.data,
  ...
  if (personalEdicionArray.length > 0) {
    edicionResp = await correspondenciaStore.editAsignacionEmpleados(
