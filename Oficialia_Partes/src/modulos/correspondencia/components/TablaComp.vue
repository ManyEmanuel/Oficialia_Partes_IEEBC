...
const { correspondencias, modalAdjuntos } = storeToRefs(correspondenciaStore);
...
onBeforeMount(() => {
  correspondenciaStore.loadCorrespondencias();
});
...
const buscarFecha = async () => {
  ...
  if (typeof model.value != "string") {
    ...
    resp = await correspondenciaStore.loadCorrespondenciasFecha(from, to);
  } else {
    resp = await correspondenciaStore.loadCorrespondenciasFecha(model.value, 0);
  }
  $q.loading.hide();
};
...
const asignar = async (id) => {
  $q.loading.show();
  await correspondenciaStore.loadPersonalArea(id);
  correspondenciaStore.actualizarEmpleado(true);
...
const crearReporte = async () => {
  $q.loading.show();
  let resp = await correspondenciaStore.createReporte(
...
const actualizarAdjunto = (valor) => {
  correspondenciaStore.actualizarAdjunto(valor);
};
