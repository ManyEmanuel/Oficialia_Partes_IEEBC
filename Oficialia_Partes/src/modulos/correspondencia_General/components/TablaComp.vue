...
onBeforeMount(async () => {
  await correspondenciaGeneralStore.loadCorrespondencias();
});
...
const buscarFecha = async () => {
  ...
  if (typeof model.value != "string") {
    ...
    resp = await correspondenciaGeneralStore.loadCorrespondenciaFecha(from, to);
  } else {
    resp = await correspondenciaGeneralStore.loadCorrespondenciaFecha(
      model.value,
      0
    );
  }
...
const verTodos = async () => {
  correspondenciaGeneralStore.loadCorrespondencias();
